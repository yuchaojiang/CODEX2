...
```{r, message=FALSE}
coverageObj <- getcoverage(bambedObj, mapqthres = 20, chr = 22)
Y <- coverageObj$Y
write.csv(Y, file = paste(projectname, '_coverage_', chr = 22, '.csv', sep=''), quote = FALSE)
head(Y[,1:5])
```

## Quality control

First, read in genome-wide data.

```{r, message= F}
Y.all <- matrix(nrow=length(ref), ncol=length(sampname))
colnames(Y.all) <- sampname
rownames(Y.all) <- paste(seqnames(ref),':',start(ref),'-',end(ref),sep='')
for(chr in unique(seqnames(ref))){
  message(chr)
  chr.index <- which(seqnames(ref)==chr)
  Y.chr <- read.csv(paste(projectname, '_coverage_', chr, '.csv', sep=''),
                    row.names = 1)
  Y.all[chr.index,] <- as.matrix(Y.chr)
}
```

Then, take a sample-wise and exon-wise quality control procedure on 
the depth of coverage matrix.
```{r, message = FALSE}
qcObj <- qc(Y.all, sampname, ref, cov_thresh = c(20, 4000),
            length_thresh = c(20, 2000), mapp_thresh = 0.9,
            gc_thresh = c(20, 80))
Y.all_qc <- qcObj$Y_qc; sampname_qc <- qcObj$sampname_qc
...
```{r, message=FALSE}
Y.nonzero <- Y.all_qc[apply(Y.all_qc, 1, function(x){!any(x==0)}),]
...
normObj.null <- normalize_null(Y_qc = Y.all_qc[chr.index,],
...
normObj <- normalize_codex2_ns(Y_qc = Y.all_qc[chr.index,],
                               gc_qc = gc_qc[chr.index], 
...
z.codex2 <- log(Y.all_qc[chr.index,]/normObj.null$Yhat[[which.max(normObj.null$BIC)]])
...
normObj <- normalize_codex2_nr(Y_qc = Y.all_qc[chr.index,], gc_qc = gc_qc[chr.index], 
...
```{r, message=FALSE}
finalcall.CBS <- segmentCBS(Y.all_qc[chr.index,],  # recommended
                            Yhat.ns, optK = which.max(BIC.ns),
                            K = 1:5,
                            sampname_qc = paste('sample',1:ncol(Y.all_qc),sep=''),
                            ref_qc = ranges(ref_qc)[chr.index],
                            chr = chr, lmax = 400, mode = "integer")

finalcall.HMM <- segmentHMM(Y.all_qc[chr.index,],  # not recommended
                            Yhat.ns, optK = which.max(BIC.ns),
                            K = 1:5,
                            sampname_qc = paste('sample',1:ncol(Y.all_qc),sep=''),
                            ref_qc = ranges(ref_qc)[chr.index],
